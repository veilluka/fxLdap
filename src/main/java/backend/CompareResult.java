...
public class CompareResult implements java.io.Serializable, Comparable<CompareResult> {
    ...
    public CompareResult(SearchResultEntry entry) {
    ...
    public CompareResult clone() {
        CompareResult compareResult = new CompareResult(_sourceEntry);
        compareResult._targetEntry = _targetEntry;
        compareResult._childrenNotEqual = _childrenNotEqual;
        compareResult.set_compare_result(get_compare_result());
        compareResult._dn = _dn;
        compareResult._parent_DN = _parent_DN;
        compareResult._entry_type = _entry_type;
        compareResult._entryEqual = _entryEqual;
        compareResult._rdnDisplay = _rdnDisplay;
        if (_modificationsToTarget != null) {
            compareResult._modificationsToTarget = new ArrayList<Modification>();
            compareResult._modificationsToTarget.addAll(_modificationsToTarget);
        }
        if (_modificationsToSource != null) {
            compareResult._modificationsToSource = new ArrayList<Modification>();
            compareResult._modificationsToSource.addAll(_modificationsToSource);
        }
        if (_allValues != null) {
            compareResult._allValues = new HashMap<>();
            compareResult._allValues.putAll(_allValues);
        }
