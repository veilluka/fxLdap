...
import backend.LdapExplorerEvent;
...
public class ShowEntryController implements ILoader {
    ...
    public void setMainApp(Main main) {
        _main = main;
        _main.get_ldapSourceExploreController()._treeView.addEventHandler(LdapExplorerEvent.ELEMENT_SELECTED, e -> {
            try {
                /*
                 clearResult();
            writeSearchResult();
            if (newValue != null && !newValue.equalsIgnoreCase("")) {
                 ...
                 */
               writeEntry(e.treeEntry, _main.get_ldapSourceExploreController().get_currentConnection());
    ...
    public void setWindow(Parent parent) {
        ...
        _showEntryWindow.getChildren().add(0,_main.get_entryView());
    }
    ...
    public void writeSearchResult(CustomEntry entry, boolean ignoreCase, boolean deadLink,
        ...
        _main.get_entryView().setSearchMode(searchValue);
        _main.get_entryView().updateValue(entry,_main.get_ldapSourceExploreController().get_currentConnection());
         clearResult();
