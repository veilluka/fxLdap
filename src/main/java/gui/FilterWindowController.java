...
public class FilterWindowController {
    ...
    private void onTextFieldFilterAttributesTextChanged(ActionEvent event)
    {
        ...
        {
            _listAllAttributes.getItems().clear();
            _listAllAttributes.getItems().addAll(_mainApp._ldapSourceExploreController.get_currentConnection().SchemaAttributes);
        }
        else
        {
            _listAllAttributes.getItems().clear();
            for(String s: _mainApp.get_configuration()._allAttributes)
            {
