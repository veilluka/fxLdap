...
public class SettingsController implements ILoader {
    ...
    public void addKeyStoreTab() {
        ...
        if (_keyStoreTab.getContent() == null) {
            _keyStoreTab.setContent(_main.get_keyStoreController()._keyStoreWindow);
        }
    ...
    public void setWindow(Parent parent) {
        ...
        _stage.onCloseRequestProperty().addListener((observable, oldValue, newValue) -> {
            if (_main == null || _main.get_keyStoreController() == null) return;
            _main.get_keyStoreController().cleanUp();
        });
        _stage.onShowingProperty().addListener((observable, oldValue, newValue) -> {
            if (_main.get_configuration().get_keyStore() == null) removeKeyStoreTab();
            else addKeyStoreTab();
            autoResizeColumns(_tableViewConnections);
            _main.get_keyStoreController().initWindow();
        });
    ...
    private void initButtons() {
        ...
        _buttonSelectBeyondCompare.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                ...
                _main.get_configuration().set_beyondCompareExe(selectedFile.getAbsolutePath());
            }
        });
        ...
        _buttonSelectTempDirectory.setOnAction(x -> {
            ...
            _main.get_configuration().set_tempDir(selectedDirectory.getAbsolutePath());
        });
        ...
        _buttonSelectKeyStore.setOnAction(x -> {
            ...
            try {
                _main.get_configuration().readKeyStoreFile(new SecureString(pass));
                _textFieldKeystoreFile.setText(file.getAbsolutePath());
                _main.get_configuration().set_keyStoreFile(file.getAbsolutePath());
                _main.get_configuration().set_keyStorePassword(new SecureString(pass));
                _main.get_keyStoreController().cleanUp();
                _main.get_keyStoreController().initWindow();
            } catch (KeyStoreException | CertificateException | NoSuchAlgorithmException | IOException | SecureStorageException e) {
        ...
        _buttonCreateKeyStore.setOnAction(x -> {
            ...
            if (pass == null) return;
            try {
                _main.get_configuration().createKeyStoreFile(new SecureString(pass), file);
                _main.get_configuration().set_keyStoreFile(file.getAbsolutePath());
                _main.get_configuration().set_keyStorePassword(new SecureString(pass));
                _textFieldKeystoreFile.setText(file.getAbsolutePath());
                _main.get_keyStoreController().cleanUp();
        ...
        _buttonSaveConnections.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                Connection c = _main.get_configuration().getConnection(_textFieldConnectionName.getText());
                if (_passwordFieldConnection.getText() != null) {
                    if (!_main.get_configuration().is_configSecured()) {
        ...
        _buttonShowPassword.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                SecureString pass = _main.get_configuration().getConnectionPassword(selected);
                if (pass != null) {
        ...
        _buttonGetCertificatesFromServer.setOnAction(x -> {
            ...
            try {
                for (Enumeration<String> e = _main.get_configuration().get_keyStore().aliases(); e.hasMoreElements(); ) {
                    String name = e.nextElement();
                    X509Certificate c = (X509Certificate) _main.get_configuration().get_keyStore().getCertificate(name);
            ...
            _main.get_keyStoreController().setCertificatesToBeAdded(foundCertificates);
            _main.get_keyStoreController()._buttonClose.setVisible(true);
    ...
    private void deleteConnection(Connection connection) throws IOException {
        ...
        _main.get_configuration().deleteConnection(connection.getName());
        for (Connection c : _connectionObservableList) {
            if (c.getName().equalsIgnoreCase(connection.getName())) {
                ...
                _main.get_configuration()._connections.clear();
                for (Connection con : _connectionObservableList) {
                    _main.get_configuration()._connections.put(con.getName(), con);
                }
            }
        }
    }
    ...
    private void updateConnection(String name) {
        ...
        if (_passwordFieldConnection.getText() != null && !_passwordFieldConnection.getText().equalsIgnoreCase(Config.PASSWORD_NOT_SET) &&
            ...
            try {
                _main.get_configuration().updatePassword(c, new SecureString(_passwordFieldConnection.getText()));
            } catch (Exception e) {
        ...
        _main.get_configuration().saveConnections();
    }
    ...
    private void getBaseDN() {
        ...
        if (_tableViewConnections.getSelectionModel().getSelectedItem() == null) {
            ...
            Connection selected = (Connection) _tableViewConnections.getSelectionModel().getSelectedItem();
            pass = _main.get_configuration().getConnectionPassword(selected);
    ...
    public void readConfiguration() {
        _textFieldBeyondCompareExe.setText(_main.get_configuration().get_beyondCompareExe());
        _textFieldTempDirectoryPath.setText(_main.get_configuration().get_tempDir());
        _textFieldKeystoreFile.setText(_main.get_configuration().get_keyStoreFile());
        for (String conName : _main.get_configuration()._connections.keySet().stream().sorted().collect(Collectors.toList())) {
            Connection con = _main.get_configuration()._connections.get(conName);
    ...
    public String[] get_CSVString(Connection connection, boolean decrypt) {
        ...
        if (decrypt) {
            try {
                stringBuilder.append(_main.get_configuration().getConnectionPassword(connection));
                stringBuilder.append(";");
            } catch (Exception e) {
