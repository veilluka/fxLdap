...
public class EntryView extends TreeTableView<EntryView.EntryValue> {
    ...
    TreeTableColumn<EntryView.EntryValue, String> nameColumn;
    TreeTableColumn<EntryView.EntryValue, String> valueColumn;
    ...
    public void setSearchMode(String searchValue) {
        if (searchValue != null && !searchValue.equalsIgnoreCase("")) {
            ...
            valueColumn.setCellFactory(column -> {
                return new TreeTableCell<EntryView.EntryValue, String>() {
                    @Override
    ...
    public void cleanUpTree() {
        ObservableList<TreeItem<EntryView.EntryValue>> children = getRoot().getChildren();
        for (TreeItem<EntryView.EntryValue> child : children) {
        ...
        setRoot(null);
        refresh();
        TreeItem<EntryView.EntryValue> rootItem = new TreeItem<>(new EntryView.EntryValue("", ""));
        setRoot(rootItem);
        refresh();
    }
    ...
    public void updateValue(CustomEntry customEntry, Connection connection) {
        ...
        EntryView.EntryValue dn = new EntryValue("dn", customEntry.getDn());
        ...
        for (String atName : attributes) {
            ...
            if (values.length > 1) {
                EntryView.EntryValue rootValue = new EntryValue(atName + "[" + values.length + "]", "");
                ...
                TreeItem<EntryView.EntryValue> item = new TreeItem<>(rootValue);
                getRoot().getChildren().add(item);
                for (int i = 0; i < values.length; i++) {
                    EntryView.EntryValue entryValue = new EntryValue(atName + "[" + i + "]", values[i]);
                    if (_searchValue != null && values[i].toLowerCase().contains(_searchValue.toLowerCase())) {
                        entryValue.set_styleName(FOUND_VALUE_STYLE);
                    }
                    TreeItem<EntryView.EntryValue> child = new TreeItem<>(entryValue);
                ...
                TreeItem<EntryView.EntryValue> item = new TreeItem<>(entryView);
                getRoot().getChildren().add(item);
            }
        }
