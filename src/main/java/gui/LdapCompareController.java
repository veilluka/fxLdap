...
public class LdapCompareController implements IProgress, ILoader {
    ...
    TreeItem<CompareResult> _observedEntry = null;
    ...
    public void setWindow(Parent parent) {
        ...
        _compareResultTab = new Tab("Compare Result", _window);
        _compareResultTab.setId("\"Compare Result");
    }
    ...
    private void initControlls() {
        ...
        _buttonSyncAllEntryAttributes.setOnAction(x->{
            ObservableList<TreeItem<CompareResult>> selected = _activeCompareTree.getSelectionModel().getSelectedItems();
    ...
    public void runCompare() {
        ...
        if (_main.get_ldapTargetExploreController().is_ldapExplorerTargetAction()) {
            if (_main.get_startLdapCompareController()._choiceBoxTargetConnection.getSelectionModel().getSelectedItem() == null &&
                    _main.get_ldapTargetExploreController().get_selectedDN() == null) {
                GuiHelper.ERROR("Connection", "Target connection not selected");
                return;
            } else {
                selectedTargetDN = _main.get_ldapTargetExploreController().get_selectedDN();
                selectedSourceDN = _main.get_ldapSourceExploreController().get_selectedDN();
            }
        } else {
            selectedTargetDN = _main.get_ldapSourceExploreController().get_selectedDN();
            if (_main.get_startLdapCompareController()._choiceBoxTargetConnection.getSelectionModel().getSelectedItem() == null) {
        ...
        try {
            if (_main.get_ldapSourceExploreController().get_currentConnection().is_fileMode()) {
                if (_main.get_ldapSourceExploreController().get_currentConnection().getEntry(selectedSourceDN) == null) {
        ...
        String rootSourceDN = _main.get_ldapSourceExploreController().get_selectedDN();
        ...
        FilterWindowController filterView = _main.get_startLdapCompareController().getEmbeddedFilterViewController();

        _currentSourceConnection = _main.get_ldapSourceExploreController().get_currentConnection();
        if (_main.get_ldapTargetExploreController().is_ldapExplorerTargetAction()) {
            _currentTargetConnection = _main.get_ldapTargetExploreController().get_currentConnection();
            rootTargetDN = _main.get_ldapTargetExploreController().get_selectedDN();
            try {
                if (_main.get_ldapTargetExploreController().get_currentConnection().is_fileMode()) {
                    if (_main.get_ldapTargetExploreController().get_currentConnection().getEntry(selectedTargetDN) == null) {
            ...
            SecureString secureString = _main.get_configuration().getConnectionPassword(_main.get_startLdapCompareController()._choiceBoxTargetConnection.getValue());
            if (secureString == null) {
                String loginPassword = GuiHelper.enterPassword("Source Connection Password", "Enter login password");

                try {
                    connection = _main.get_ldapSourceExploreController()._choiceBoxEnviroment.getValue().copy();
                ...
                try {
                    connection = _main.get_startLdapCompareController()._choiceBoxTargetConnection.getValue().copy();
        ...
        _main.get_startLdapCompareController()._stage.close();
        if (_main.get_startLdapCompareController()._checkBoxIgnoreMissingEntries.isSelected())
            _activeCompareTree.set_ignoreMissingEntries(true);
        else _activeCompareTree.set_ignoreMissingEntries(false);
        if (_main.get_startLdapCompareController()._checkBoxIgnoreOperationalAttributes.isSelected())
        ...
        _progressController._labelHeader.setText(_main.get_ldapSourceExploreController().get_selectedDN());
        _progressStage.show();
        ...
        try {
            _activeCompareTree.initCompare(
                    ...
                    _main.get_startLdapCompareController()._checkBoxShowAllEntries.isSelected(),
                    _main.get_startLdapCompareController()._checkBoxIgnoreWhiteSpace.isSelected(),
                    filterView.get_sourceFilter(),
                    _main.get_configuration(),
                    ...
                    _main.get_startLdapCompareController()._checkBoxSubtree.isSelected());
    ...
    ChangeListener<TreeItem<CompareResult>> listener = new ChangeListener<TreeItem<CompareResult>>() {
        @Override
        public void changed(ObservableValue<? extends TreeItem<CompareResult>> observable,
                            TreeItem<CompareResult> oldValue, TreeItem<CompareResult> newValue) {
            ...
            if (newValue != null) {
                ...
                _entryDiffView.updateValue(_observedEntry.getValue(), _checkBoxShowEqual.isSelected());
                if (_observedEntry.getValue().get_entry_type().equals(CompareResult.ENTRY_TYPE.SOURCE)) {
                    ...
                    if (_observedEntry.getParent() != null) {
                        if (_observedEntry.getParent().getValue().get_entry_type().equals(CompareResult.ENTRY_TYPE.BOTH)) {
                ...
                } else if (_observedEntry.getValue().get_entry_type().equals(CompareResult.ENTRY_TYPE.TARGET)) {
                ...
                else if(_observedEntry.getValue().get_entry_type().equals(CompareResult.ENTRY_TYPE.BOTH))
                {
                    if(_observedEntry.getValue().get_compare_result().equals( CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL))
                    {
            ...
            if (e.getButton() == MouseButton.SECONDARY) {
                ...
                if (_observedEntry.getValue().get_entry_type().equals(CompareResult.ENTRY_TYPE.SOURCE)) {
                    if (_observedEntry.getParent() != null) {
                        ...
                        try {
                            Entry rootEntry = _main.get_ldapSourceExploreController().get_currentConnection().getEntry(_activeCompareTree.getRoot().getValue().get_dn());
                            DN parentDN = rootEntry.getParentDN();
                            if (parentDN != null) {
                                if (_currentTargetConnection.getEntry(parentDN.toString()) != null) {
                                    _entryContextMenu.getItems().add(_entryMenuCopyToTarget);
    ...
    public void setMainApp(Main main) {
        ...
        _entryMenuBeyondCompare.setOnAction(event -> {
            String error = null;
            if (_main.get_SettingsController()._textFieldTempDirectoryPath.getText() == null)
                error = "Temp directory not set in settings, set it first!";
            if (_main.get_SettingsController()._textFieldBeyondCompareExe.getText() == null)
                error = "Beyond compare executable not set in config!";
            if (error != null) {
                GuiHelper.ERROR("Error Beyond Compare", error);
            } else {

                String sourceFileName = _main.get_SettingsController()._textFieldTempDirectoryPath.getText() + "\\" +
                        _currentSourceConnection.getName() + "_" + _observedEntry.getValue().getRDN() + ".ldif";
                String targetFileName = _main.get_SettingsController()._textFieldTempDirectoryPath.getText() + "\\" +
                        _currentTargetConnection.getName() + "_" + _observedEntry.getValue().getRDN() + ".ldif";
                List<TreeItem<CompareResult>> diffNodes =
                ...
                try {
                    ProcessBuilder pb = new ProcessBuilder(
                            "cmd",
                            "\"/c\"",
                            "\"" + _main.get_SettingsController()._textFieldBeyondCompareExe.getText() + "\"",
                            "\"" + sourceFileName + "\"",
                            "\"" + targetFileName + "\""
                    );
                    pb.start();
    ...
    {
        ObservableList<TreeItem<CompareResult>> selectedEntries = null;
        List<TreeItem<CompareResult>> allEntries = null;
        ...
        List<TreeItem<CompareResult>> filteredResult = new ArrayList<>();
        for(TreeItem<CompareResult> resultTreeItem : allEntries)
        {
            if(resultTreeItem.getValue().get_compare_result().equals(CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL))
            {
        ...
        _main.get_progressWindowController().clearProgressWindow();
        ...
        Runnable runnableTask = () -> {
            try {
                ...
                int done = 0;
                for(TreeItem<CompareResult> item: filteredResult)
                {
                    _activeCompareTree.overwriteEntryValue(item.getValue(),attName,true);
                    done++;
                    final double progressNr = (double)done/(double)nrOfEntries;
                    Platform.runLater(()->_main.get_progressWindowController().setProgress(progressNr,item.getValue().getRDN() + " OK"));
                }
                Platform.runLater(()->
                {
                    try {
                        for(TreeItem<CompareResult> item: filteredResult)
                        {
                            ...
                            {
                                logger.info("selected tree is attribute tree, take main now ");
                                CompareTree mainTree = _compareTrees.get(MAIN_TREE_ID);
                                TreeItem<CompareResult> copyOfSelectedEntry = mainTree.searchTreeItem(mainTree.getRoot(),item.getValue().get_dn());
                            ...
                            {
                                logger.info("selected tree is main tree, take attribute tree");
                                CompareTree attributeTree = _compareTrees.get(attName);
                                if(attributeTree != null)
                                {
                                    TreeItem<CompareResult> copyOfSelectedEntry = attributeTree.searchTreeItem(attributeTree.getRoot(),item.getValue().get_dn());
        ...
        try {
            future.get();
            _main.get_progressWindowController()._stage.hide();
    ...
    private void initProgressWindow(Main main) throws IOException {
        ...
        settingsLoader.setLocation(SettingsController.class.getResource("/FXML/ProgressWindow.fxml"));
    ...
    void initFilterWindow() throws IOException {
        ...
        loader_2.setLocation(Main.class.getResource("/FXML/FilterWindow.fxml"));
        _filterWindow = (VBox) loader_2.load();
        _filterWindowController = loader_2.getController();
        _filterWindowController.set_mainApp(_main);
        _filterWindowController.get_observableConfigAllAttributes().addAll(_main.get_configuration()._allAttributes);
    ...
    public void signalTaskDone(String taskName, String description, Exception e) {
        Platform.runLater(() -> {
            if (taskName.equalsIgnoreCase("compare")) {
                ...
                _main._dockWindows.dock(true, null, _main._dockWindows._dockNodeCompareResult);

            }
            if (taskName.equalsIgnoreCase("copy")) {
                try {
                    _progressController.setProgress(0.9, "Reevaluating tree now stage 1 ");
                    _activeCompareTree.reEvaluateCompareResultSubtree(_observedEntry, _main.get_ldapSourceExploreController().get_currentConnection().getDisplayAttribute());
    ...
    private boolean confirmLdapModification(HashMap<TreeItem<CompareResult>, List<String>> modifications, String header, boolean target) {
        ...
        for (TreeItem<CompareResult> cr : modifications.keySet()) {
    ...
    public void copyAttributeValueToTarget(List<String> attributeNames) {
        ObservableList<TreeItem<CompareResult>> selectedEntries = _activeCompareTree.getSelectionModel().getSelectedItems();
        HashMap<TreeItem<CompareResult>, List<String>> entriesToBeModified = new HashMap<>();
        List<TreeItem<CompareResult>> allSelected = new ArrayList<>();
        for (TreeItem<CompareResult> selectedEntry : selectedEntries) {
            allSelected.add(selectedEntry);
            List<TreeItem<CompareResult>> children = null;
        ...
        for (String attributeName : attributeNames) {
            ...
            for (TreeItem<CompareResult> selectedEntry : allSelected) {
        ...
        if (confirmLdapModification(entriesToBeModified, "Copy " + attributeNames + System.lineSeparator() + " from " +
            ...
            for (TreeItem<CompareResult> cr : entriesToBeModified.keySet())
                for (String attributeName : entriesToBeModified.get(cr)) {
                    try {
                        _activeCompareTree.overwriteEntryValue(cr.getValue(), attributeName, true);
                        _activeCompareTree.reEvaluateCompareResult(cr, _main.get_ldapSourceExploreController().get_currentConnection().getDisplayAttribute());
                    } catch (Exception e) {
    ...
    private void createCopyWithSearchAttributes(TreeItem<CompareResult> source, TreeItem<CompareResult> target, List<String> attributes) {
        for (TreeItem<CompareResult> s : source.getChildren()) {
            CompareResult cloned = s.getValue().clone();
            logger.info("Evaluate ->" + cloned.getRDN());
            if (cloned.get_compare_result().equals(CompareResult.COMPARE_RESULT.ONLY_IN_SOURCE)) {
                logger.info("only in source->" + cloned.getRDN());
                continue;
            }
            if (cloned.get_compare_result().equals(CompareResult.COMPARE_RESULT.ONLY_IN_TARGET)) {
                logger.info("only in target->" + cloned.getRDN());
                continue;
            }
            if (cloned.get_compare_result().equals(CompareResult.COMPARE_RESULT.ENTRY_EQUAL)) {
                logger.info("skip equal->" + cloned.getRDN());
                continue;
            }
            if (cloned.get_compare_result().equals(CompareResult.COMPARE_RESULT.ENTRY_EQUAL_BUT_CHILDREN_NOT)) {
                logger.info("children not equal for->" + cloned.getRDN());
                TreeItem<CompareResult> targetEntry = new TreeItem<>(cloned);
                target.getChildren().add(targetEntry);
                createCopyWithSearchAttributes(s, targetEntry, attributes);
            } else if (cloned.get_compare_result().equals(CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL)) {
                logger.info("check not equal ->" + cloned.getRDN());
                ...
                if (mod2Source.isEmpty() && mod2Target.isEmpty()) {
                    ...
                    cloned.set_compare_result(CompareResult.COMPARE_RESULT.ENTRY_EQUAL);
                    cloned.set_entryEqual(true);
                    TreeItem<CompareResult> targetEntry = new TreeItem<>(cloned);
                    target.getChildren().add(targetEntry);
                    createCopyWithSearchAttributes(s, targetEntry, attributes);
                } else {
                    logger.info("entry not  equal->" + cloned.getRDN());
                    TreeItem<CompareResult> targetEntry = new TreeItem<>(cloned);
                    target.getChildren().add(targetEntry);
                    createCopyWithSearchAttributes(s, targetEntry, attributes);
                }
