...
import backend.CollectionEntry;
...
public class EntryDiffView extends TreeTableView<EntryDiffView.EntryValue> {
    ...
    TreeTableColumn<EntryDiffView.EntryValue, String> nameColumn;
    TreeTableColumn<EntryDiffView.EntryValue, String> resultColumn;
    TreeTableColumn<EntryDiffView.EntryValue, String> valueColumnSource;
    TreeTableColumn<EntryDiffView.EntryValue, String> valueColumnTarget;

    Main _mainApp;
    private static CompareResult _currentCompareResult = null;
    ...
    public EntryDiffView(Main main) {
        ...
        resultColumn.setCellFactory(param -> new TreeTableCell<EntryValue, String>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (item.equalsIgnoreCase(CompareResult.COMPARE_RESULT.ONLY_IN_TARGET.toString()))
                    setGraphic(Icons.get_iconInstance().getIcon(Icons.ICON_NAME.ARROW_LEFT));
                else if (item.equalsIgnoreCase(CompareResult.COMPARE_RESULT.ONLY_IN_SOURCE.toString()))
                    setGraphic(Icons.get_iconInstance().getIcon(Icons.ICON_NAME.ARROW_RIGHT));
                else if (item.equalsIgnoreCase(CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL.toString()))
                    setGraphic(Icons.get_iconInstance().getIcon(Icons.ICON_NAME.UNEQUAL));
                else if (item.equalsIgnoreCase(CompareResult.COMPARE_RESULT.ENTRY_EQUAL.toString()))
        ...
        getSelectionModel().selectedItemProperty()
                .addListener((observable, oldValue, newValue) -> {
                    if (newValue == null) {
                        _mainApp.get_ldapCompareController().set_attributeDifferences(null, null);
                    } else {
                        _mainApp.get_ldapCompareController().set_attributeDifferences(
        ...
        addEventHandler(MouseEvent.MOUSE_RELEASED, e -> {
            if (e.getButton() == MouseButton.SECONDARY) {
                ...
                if (item.getValue().resultProperty.get().equalsIgnoreCase(CompareResult.COMPARE_RESULT.ENTRY_EQUAL.toString()))
                    return;
        ...
        _attributeMenuCopyValueToRight.setOnAction(x -> {
            ...
            _mainApp.get_ldapCompareController().copyAttributeValueToTarget(attributes);
        });
        ...
        widthProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                if (_currentCompareResult != null) {
                    StringBuilder builder = new StringBuilder();
                    _currentCompareResult.getDifferentAttributes().stream().sorted().forEach(x -> {
    ...
    public void updateValue(CompareResult compareResult, boolean showEquals) {
        _currentCompareResult = compareResult;
        StringBuilder builder = new StringBuilder();
        builder.append(nameColumn.getText());
        cleanUpTree();
        Entry sourceEntry = compareResult.get_sourceEntry();
        Entry targetEntry = compareResult.get_targetEntry();
        ...
        if (sourceEntry == null) {
            ...
            if (showEquals) {
                ...
                evalAttributes = compareResult.getDifferentAttributes();
            }
        }
        evalAttributes.removeAll(_mainApp.get_ldapCompareController()._currentSourceConnection.getOperationalAttributes());
        evalAttributes.removeAll(_mainApp.get_ldapCompareController()._currentTargetConnection.getOperationalAttributes());
        for (String x : evalAttributes) {
            ...
            if (!multiValue) {
                ...
                String state = "";
                if (sourceValue.equalsIgnoreCase("")) state = CompareResult.COMPARE_RESULT.ONLY_IN_TARGET.toString();
                else if (targetValue.equalsIgnoreCase(""))
                    state = CompareResult.COMPARE_RESULT.ONLY_IN_SOURCE.toString();
                else {
                    if (sourceValue.equals(targetValue)) state = CompareResult.COMPARE_RESULT.ENTRY_EQUAL.toString();
                    else state = CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL.toString();
                }
                getRoot().getChildren().add(new TreeItem<>(new EntryValue(x, state, sourceValue, targetValue)));
            } else {
                String attState = "";
                if (compareResult.getDifferentAttributes().contains(x)) {
                    attState = CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL.toString();
                } else if (targetAttribute == null) {
                    attState = CompareResult.COMPARE_RESULT.ONLY_IN_SOURCE.toString();
                } else if (sourceAttribute == null) {
                    attState = CompareResult.COMPARE_RESULT.ONLY_IN_TARGET.toString();
                } else {
                    attState = CompareResult.COMPARE_RESULT.ENTRY_EQUAL.toString();
                }
                ...
                if (sourceValues.isEmpty()) {
                    ...
                    for (String v : targetValues) {
                        TreeItem<EntryValue> valueNode = new TreeItem<>(new EntryValue("", CompareResult.COMPARE_RESULT.ONLY_IN_TARGET.toString(), "", v));
                        attributeNode.getChildren().add(valueNode);
                    }
                } else if (targetValues.isEmpty()) {
                    for (String v : sourceValues) {
                        TreeItem<EntryValue> valueNode = new TreeItem<>(new EntryValue("", CompareResult.COMPARE_RESULT.ONLY_IN_SOURCE.toString(), v, ""));
                        attributeNode.getChildren().add(valueNode);
                    }
                } else {
                    ...
                    for (String v : sourceValues) {
                        ...
                        String state = "";
                        if (same) state = CompareResult.COMPARE_RESULT.ENTRY_EQUAL.toString();
                        else if (onlySource) state = CompareResult.COMPARE_RESULT.ONLY_IN_SOURCE.toString();
                        else if (onlyTarget) state = CompareResult.COMPARE_RESULT.ONLY_IN_TARGET.toString();
                        else if (nEqual) state = CompareResult.COMPARE_RESULT.ENTRY_NOT_EQUAL.toString();
        ...
        if (_mainApp.get_ldapCompareController()._currentSourceConnection != null)
            valueColumnSource.setText(_mainApp.get_ldapCompareController()._currentSourceConnection.getName());
        if (_mainApp.get_ldapCompareController()._currentTargetConnection != null)
            valueColumnTarget.setText(_mainApp.get_ldapCompareController()._currentTargetConnection.getName());
