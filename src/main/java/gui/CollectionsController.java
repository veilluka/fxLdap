...
public class CollectionsController implements IProgress, ILoader {
    ...
    void initFilterWindow() throws IOException {
        ...
        loader.setLocation(Main.class.getResource("/FXML/FilterWindow.fxml"));
    ...
    private void initialize() {
        ...
        embeddedProjectViewController._buttonImportLDIF.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                fileChooser.setInitialDirectory(new File(_main.get_configuration().get_lastUsedDirectory()));
                File selectedFile = fileChooser.showOpenDialog(_main.get_primaryStage());
                if (selectedFile == null) return;
                _main.get_configuration().set_lastUsedDirectory(selectedFile.getParent());
        ...
        embeddedProjectViewController._buttonExportLDIF.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                _main.get_configuration().set_lastUsedDirectory(selectedDirectory.getAbsolutePath());
                _progressStage.show();
                ExecutorService executor = Executors.newSingleThreadExecutor();
                executor.submit(() -> {
                    try {
            ...
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                ...
                if (_observedEntry.isExpanded()) {
                    if (_observedEntry.getChildren().size() == 1 && _observedEntry.getChildren().get(0).getValue().is_dummy()) {
                        ...
                        _currentReader = new UnboundidLdapSearch(_main.get_configuration(),_currentConnection, _observedEntry.getValue().getEntry().getDN(), null, _collectionController);
    ...
    private void addProjectEntriesToTree(CollectionEntry selectedEntry) {
        ...
        for (int i = dnSplit.length - 1; i >= 0; i--) {
            ...
            if (f != null) {
                ...
                UnboundidLdapSearch unboundidLdapSearch = new UnboundidLdapSearch(_main.get_configuration(),_currentConnection, searchDN, null, _collectionController);
    ...
    public boolean newProject() {
        ...
        _currentCollectionsProject = new CollectionsProject(_main.get_configuration(),projectName);
    ...
    public void connect(Connection selectedConnection) {
        ...
        password = _main.get_configuration().getConnectionPassword(selectedConnection);
        ...
        try {
            ...
            _currentReader = new UnboundidLdapSearch(_main.get_configuration(),_currentConnection, _currentConnection.getBaseDN(), null, _collectionController);
            _currentReader.setDisplayAttribute(_currentConnection.getDisplayAttribute());
